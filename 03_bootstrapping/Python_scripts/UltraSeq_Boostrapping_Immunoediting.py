...
def Bootstrapping_Final_df_v1(raw_df,input_sample_list1,input_sample_list2,cell_number_cutoff_focal,cell_number_cutoff_ref,percentile_list,number_of_replicate,input_total_gRNA_number):
    ...
    temp_final_df_observed_gene['Bootstrap_id'] = ['Real']*temp_final_df_observed_gene.shape[0] # gene df
    ...
    if number_of_replicate!=0:
        ...
        for bootstrap_cycle in range(number_of_replicate):
            ...
            temp_out_dfs_gene.append(temp_metric_df_gene)

    temp_out_df = pd.concat(temp_out_dfs, ignore_index=True)
    temp_out_df_gene = pd.concat(temp_out_dfs_gene, ignore_index=True)

    Inert_gene_number = len(temp_ref_df1[temp_ref_df1.Type=='Inert']['Targeted_gene_name'].unique())
...
def Bootstrapping_Final_df_v2(raw_df,input_sample_list1,input_sample_list2,cell_number_cutoff_focal,cell_number_cutoff_ref,percentile_list,number_of_replicate,input_total_gRNA_number,minimal_tumor_size):
    ...
    # Control mouse
    temp_ref_df2 = Generate_ref_input_df(raw_df,input_sample_list2,cell_number_cutoff_ref).copy()
    temp_summary = temp_ref_df2.groupby(['gRNA','Targeted_gene_name','Type'],as_index=False)['Count'].count()
    ...
    sgRNA_ratio_dic = dict(zip(temp_summary.gRNA,temp_summary.Count/temp_total_inert_number))
    ...
    temp_summary = temp_ref_df1_sub.groupby(['gRNA','Targeted_gene_name','Type'],as_index=False)['Count'].count()
    temp_total_inert_number =  temp_summary[temp_summary.Type=='Inert']['Count'].sum()
    ...
    temp_final_df_observed = Calculate_Relative_Normalized_Metrics(temp_ref_df1_new,None,percentile_list,'gRNA')
    temp_final_df_observed_gene = Calculate_Relative_Normalized_Metrics(temp_ref_df1_new,None,percentile_list,'Targeted_gene_name')
    temp_final_df_observed['Bootstrap_id'] = ['Real']*temp_final_df_observed.shape[0] #gRNA df
    temp_final_df_observed_gene['Bootstrap_id'] = ['Real']*temp_final_df_observed_gene.shape[0] # gene df
...
def Bootstrapping_by_Plasmid_Final_df_v1(raw_df,plasmid_df,input_sample_list1,cell_number_cutoff_focal,percentile_list,number_of_replicate,input_total_gRNA_number):
    ...
    temp_final_df_observed = Calculate_Relative_Normalized_Metrics_by_Plasmid(temp_ref_df1,temp_ref_df2,percentile_list,'gRNA')
    temp_final_df_observed_gene = Calculate_Relative_Normalized_Metrics_by_Plasmid(temp_ref_df1,temp_ref_df2,percentile_list,'Targeted_gene_name')
    ...
    if number_of_replicate!=0:
        ...
        for bootstrap_cycle in range(number_of_replicate):
            x = Nested_Boostrap_Index_single(Mouse_index_dic_1)
            temp_bootstrap_df_1 = temp_ref_df1.loc[x]
            temp_metric_df = Calculate_Relative_Normalized_Metrics_by_Plasmid(temp_bootstrap_df_1,temp_ref_df2,percentile_list,'gRNA')
            temp_metric_df_gene = Calculate_Relative_Normalized_Metrics_by_Plasmid(temp_bootstrap_df_1,temp_ref_df2,percentile_list,'Targeted_gene_name')
            temp_metric_df['Bootstrap_id'] = ['B'+str(bootstrap_cycle)]*temp_metric_df.shape[0]
            temp_metric_df_gene['Bootstrap_id'] = ['B'+str(bootstrap_cycle)]*temp_metric_df_gene.shape[0]
            temp_out_dfs.append(temp_metric_df)
            temp_out_dfs_gene.append(temp_metric_df_gene)
...
def Bootstrapping_by_Plasmid_Final_df_v2(raw_df,plasmid_df,input_sample_list1,cell_number_cutoff_focal,percentile_list,number_of_replicate,input_total_gRNA_number,minimal_tumor_size):
    ...
    temp_summary = temp_ref_df2.groupby(['gRNA','Type'],as_index=False)['Count'].count()
    temp_total_inert_number =  temp_summary[temp_summary.Type=='Inert']['Count'].sum()
    # I want to get the relationship between tumor number and inert tumor number
    sgRNA_ratio_dic = dict(zip(temp_summary.gRNA,temp_summary.Count/temp_total_inert_number))
    # Get the expected tumor number for each sgRNA
...
def Generate_AC_data(df1,minimal_tumor_size,gRNA_dic):
    ...
    for gRNA, X in gRNA_dic.items():
        # Filter df1 for rows with the current gRNA and sort by Cell_number
        gRNA_df = df1[df1['gRNA'] == gRNA].sort_values(by='Cell_number', ascending=False)
        ...
        if num_available >= X:
            ...
            if not gRNA_df.empty:
                ...
                print(f"Warning: {gRNA} is missing in the tumor data")
...
def Calculate_Relative_Normalized_Metrics(input_df1,input_df2, percentile_list,group_trait='gRNA'):
    ...
    if group_trait=='gRNA':
        temp_df = temp_df.merge(input_df1[['gRNA','Targeted_gene_name', 'Numbered_gene_name']].drop_duplicates(),how = 'inner',on = 'gRNA')
    return(temp_df)
