...
def main():
    parser = argparse.ArgumentParser(description='A function to extract gRNA and clonal barcode from merged fastq gz file for the immunoediting project')
    ...
    with gzip.open(fastqgz_input_address,'rt') as handler:
        ...
        while temp_readID:
            ...
            temp_pattern = regex.compile('(TGGAC){e<2}' + '(.{4})' + 'CACC'+'(.{10})' + 'G' + '(.{20,21})' + 'GTT(TTAGA){e<2}')
            temp_search_result = temp_pattern.search(temp_sequence)
            if temp_search_result:
                temp_vector_id = temp_search_result.group(2)
                if temp_vector_id in ref_id: # I require both crietrias to be satisfied
                    temp_extracted_read+=1
                    temp_gRNA = temp_search_result.group(4)
                    temp_clonal_barcode = temp_search_result.group(2) + temp_search_result.group(3) # clonal barcode is a vector id plus barcode 
