...
def Combine_sgRNA_barcode_from_the_Same_mouse(input_folder_address):
    ...
    # modification for immunoediting
    vector_id_list = [x[:4] for x in temp_final_completely_deduplexed.Clonal_barcode]
    barcode_list = [x[4:] for x in temp_final_completely_deduplexed.Clonal_barcode]
    temp_final_completely_deduplexed['Vector_ID'] = vector_id_list
    temp_final_completely_deduplexed['Clonal_barcode'] = barcode_list
