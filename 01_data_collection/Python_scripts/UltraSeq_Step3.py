...
def main():
    ...
    df1_final = df1_final[df1_final.Center.isin(gRNA_df.gRNA)]
    ...
    for groups in sgRNA_groups:
        ...
        # find the frequency correpsonding to each clonal barcode 
        temp_summary_df = value.groupby('Clonal_barcode',as_index=False)['Read_ID'].count()
        temp_final_list = temp_summary_df[temp_summary_df.Read_ID>1]['Clonal_barcode'].to_list()
        file_a.write(temp_name + '\n')
        value[value.Clonal_barcode.isin(temp_final_list)][['Clonal_barcode', 'Read_ID']].to_csv(temp_name, sep=',', header=False, index=False)
    file_a.close()
